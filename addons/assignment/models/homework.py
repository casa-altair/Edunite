...
class SchoolTeacherAssignment(models.Model):
    ...
    name = fields.Char(string="Assignment Name", help="Name of Assignment")
    subject_id = fields.Many2one(
        "subject.subject", "Subject", required=True, help="Select Subject"
    )
    standard_id = fields.Many2one("school.standard", "Class", help="Select Standard")
    teacher_id = fields.Many2one(
        "school.teacher", "Teacher", required=True, help="Select Teacher"
    )
    assign_date = fields.Date(required=True, help="Starting date of assignment")
    due_date = fields.Date(required=True, help="Ending date of assignment")
    attached_homework = fields.Binary("Attached Home work", help="Attached Homework")
    ...
    file_format = fields.Many2one("file.format", help="File format")
    attach_files = fields.Char(string="File Name", help="Enter file name")
    ...
    def active_assignment(self):
        ...
        for rec in self:
            ...
            for std in student_obj.search(
                [("standard_id", "=", rec.standard_id.id), ("state", "=", "done")]
            ):
...
class SchoolStudentAssignment(models.Model):
    ...
    name = fields.Char(string="Assignment Name", help="Assignment Name")
    subject_id = fields.Many2one(
        "subject.subject", "Subject", required=True, help="Select Subject"
    )
    ...
    assign_date = fields.Date(required=True, help="Starting date of assignment")
    due_date = fields.Date(required=True, help="End date of assignment")
    state = fields.Selection(
        [
            ("draft", "Draft"),
            ("active", "Active"),
    ...
    student_standard = fields.Many2one(
        "standard.standard", help="Select student standard"
    )
    ...
    file_name = fields.Char(help="Enter file name")
    active = fields.Boolean(default=True, help="Activate/Deactivate assignment")
    ...
    def check_file_format(self):
        if self.file_name:
            ...
            if len(file_format) == 2:
                ...
                raise ValidationError(
                    _("Kindly attach file with format: %s!") % self.attachfile_format
                )
            if (
    ...
    def done_assignment(self):
        ...
        if self.submission_type == "softcopy" and not self.submit_assign:
            raise ValidationError(
                _("You have not attached the homework! Please attach the " "homework!")
            )
        self.state = "done"
...
class FileFormat(models.Model):
    ...
    name = fields.Char(
        string="File Name ", help="Enter file format that can be attached"
    )
